...
def grad_cam(image, model, layer_name):     # Analyse une seule image à chaque fois
    ...
    grad_model = Model(
    inputs=model.input,
    outputs=[layer.output, model.output if not isinstance(model.output, list) else model.output[0]]
    )
...
def get_predictions_and_labels2(dataset):
    true_labels = []
    pred_labels = []
