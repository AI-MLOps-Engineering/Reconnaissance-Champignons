...
from functions2 import load_df_main_dataset, load_df_dataset_30, load_data, loading_model, load_training_history, get_predictions_and_labels, conf_mat_affichage, training_curves, affichage_top3, show_grad_cam, grad_cam, shap_function, show_feature_maps, get_model_summary, dico_lat_fra, images_show, barplot_display, countplot_display, show_n_images, SKLearnKerasClassifier
from settings2 import rep_data, rep_img, rep_img_2, rep_img_3, rep_model, models, main_dataset, dataset_30, taxons, n_img_show, dico_taxons
...
if model_choice == "Ensemble":
    models_cnn_loaded = []
    models_cnn_loaded.append(tf.keras.models.load_model(rep_model + "cnn_tl_model.keras", compile=False))
    models_cnn_loaded.append(tf.keras.models.load_model(rep_model + "cnn_tl_model_efficientb0.keras", compile=False))
    models_cnn_loaded.append(tf.keras.models.load_model(rep_model + "cnn_tl_model_nasnet.keras", compile=False))
    model_type = "ensemble"
else:
...
if menu == "Accueil":
    ...
    st.markdown(
                    ...
                    <p style='color:#000000; font-size:25px; font-weight:1000; text-align: left'>
                		...
                		&emsp;RENVOISÉ Quentin (DS Avr-25), <br>
                        &emsp;PREGASSAME Saravana (MLE Jan-25)
                    </p>                    
                    """,
                    unsafe_allow_html=True
                )
...
if menu == "Modèles":
    ...
    if model_type == "sklearn": # RF, KNN
    ...
    elif model_type == "keras":   # Si modèle Keras (DNN, CNN)
    ...
    else:  #Modèle Ensemble

        models_cnn_loaded[0].trainable = False
        models_cnn_loaded[1].trainable = False
        models_cnn_loaded[2].trainable = False

        wrapped_resnet = SKLearnKerasClassifier(models_cnn_loaded[0])
        wrapped_efficientnetb0 = SKLearnKerasClassifier(models_cnn_loaded[1])
        wrapped_nasnet = SKLearnKerasClassifier(models_cnn_loaded[2])
        ...
        y_pred_1 = np.array(pred_labels) 
        y_val = np.array(true_labels)
    ...
    if model_type == "sklearn":
        ...
        if uploaded_file is not None:
            ...
            if model_type == "ensemble":
                ...
                preds_resnet = wrapped_resnet.predict_proba(img_tensor)
                preds_efficientnetb0 = wrapped_efficientnetb0.predict_proba(img_tensor)
                preds_nasnet = wrapped_nasnet.predict_proba(img_tensor)

                avg_predictions = (preds_resnet + preds_efficientnetb0 + preds_nasnet) / 3

                top3_resnet = np.argsort(preds_resnet, axis=-1)[:, -3:][:, ::-1]
                top3_resnet_scores = np.take_along_axis(preds_resnet, top3_resnet, axis=-1)
                top3_efficientnetb0 = np.argsort(preds_efficientnetb0, axis=-1)[:, -3:][:, ::-1]
                top3_efficientnetb0_scores = np.take_along_axis(preds_efficientnetb0, top3_efficientnetb0, axis=-1)
                top3_nasnet = np.argsort(preds_nasnet, axis=-1)[:, -3:][:, ::-1]
                top3_nasnet_scores = np.take_along_axis(preds_nasnet, top3_nasnet, axis=-1)
                top3_idx = np.argsort(avg_predictions, axis=-1)[:, -3:][:, ::-1]
                top3_scores = np.take_along_axis(avg_predictions, top3_idx, axis=-1) 
                top3_resnet = top3_resnet[0].tolist()
                top3_resnet_scores = top3_resnet_scores[0].tolist()
                top3_efficientnetb0 = top3_efficientnetb0[0].tolist()
                top3_efficientnetb0_scores = top3_efficientnetb0_scores[0].tolist()
                top3_nasnet = top3_nasnet[0].tolist()
                top3_nasnet_scores = top3_nasnet_scores[0].tolist()
                top3_idx = top3_idx[0].tolist()
                top3_scores = top3_scores[0].tolist()


                st.image(
                        img_tensor[0].numpy().astype("uint8"),
                        width=200
                    )

                st.markdown(
                    f"""
                    <p style='color:black; font-size:18px; font-weight:600;'>
                            {img_name}
                    </p>
                    """,
                    unsafe_allow_html=True
                ...
                st.markdown(
                    f"""
                    <p style='color:#004B23; font-size:22px; font-weight:600;'>
                        Prédiction des modèles CNN 
                    </p>
                    """,
                    unsafe_allow_html=True
                )

                cols = st.columns(4) # Il y a 3 modèle CNN : ResNet5, EfficientNetB0 et NasNet
                
                for n, col in enumerate(cols): # Chaque colonne correspond à un modèle CNN

                    if n == 0:
                        model_cnn = "Ensemble"
                        pred1 = class_names[top3_idx[0]]
                        pred2= class_names[top3_idx[1]]
                        pred3 = class_names[top3_idx[2]]
                        score1 = top3_scores[0]
                        score2 = top3_scores[1]
                        score3 = top3_scores[2]
                    elif n == 1 :
                        model_cnn = "ResNet50"
                        pred1 = class_names[top3_resnet[0]]
                        pred2= class_names[top3_resnet[1]]
                        pred3 = class_names[top3_resnet[2]]
                        score1 = top3_resnet_scores[0]
                        score2 = top3_resnet_scores[1]
                        score3 = top3_resnet_scores[2]
                    elif n == 2 :
                        model_cnn = "EfficientNetB0"
                        pred1 = class_names[top3_efficientnetb0[0]]
                        pred2= class_names[top3_efficientnetb0[1]]
                        pred3 = class_names[top3_efficientnetb0[2]]
                        score1 = top3_efficientnetb0_scores[0]
                        score2 = top3_efficientnetb0_scores[1]
                        score3 = top3_efficientnetb0_scores[2]
                    else:
                        model_cnn = "NasNet"
                        pred1 = class_names[top3_nasnet[0]]
                        pred2= class_names[top3_nasnet[1]]
                        pred3 = class_names[top3_nasnet[2]]
                        score1 = top3_nasnet_scores[0]
                        score2 = top3_nasnet_scores[1]
                        score3 = top3_nasnet_scores[2]

                    with col:
                        st.markdown(
                            f"""
                            <p style='color:#004B23; font-size:20px; font-weight:600;'>
                                Modèle {model_cnn}
                            </p>
                            """,
                            unsafe_allow_html=True
                        ...
                        st.markdown(
                            ...
                            ">
                                ...
                                " 
                                \n
                                Prédiction 1 : {pred1}, Score : {round(float(score1), 2)}\n
                                Prédiction 2 : {pred2}, Score : {round(float(score2), 2)}\n
                                Prédiction 3 : {pred3}, Score : {round(float(score3), 2)}\n
                            """,
                            unsafe_allow_html=True
                        )
            ...
            if model_type == "ensemble":
                ...
                conv_layers = []
                conv_layers.append([layer.name for layer in models_cnn_loaded[0].layers if isinstance(layer, Conv2D)])
                conv_layers.append([layer.name for layer in models_cnn_loaded[1].layers if isinstance(layer, Conv2D)])
                conv_layers.append([layer.name for layer in models_cnn_loaded[2].layers if isinstance(layer, Conv2D)])

                max_len = max(len(conv_layers[0]), len(conv_layers[1]), len(conv_layers[2]))

                conv_layers_choice = [f"couche {i+1}" for i in range(max_len)]

                selected_layer = st.selectbox("Choisir une couche convolutionnelle :", conv_layers_choice)
                index_selected_layer = conv_layers_choice.index(selected_layer)

                cols = st.columns(3) # Il y a 3 modèle CNN : ResNet5, EfficientNetB0 et NasNet
                ...
                for n, col in enumerate(cols): # Chaque colonne correspond à un modèle CNN
                    ...
                    with col:
                        ...
                        if index_selected_layer < len(conv_layers[0]) :

                            grad_cam_image, predicted_class = grad_cam(img_tensor[0], models_cnn_loaded[n], conv_layers[n][index_selected_layer])

                            st.markdown(
                                f"""
                                <p style='color:#004B23; font-size:14px; font-weight:600;'>
                                    {conv_layers[n][index_selected_layer]}
                                </p>
                                """,
                                unsafe_allow_html=True
                            )
            ...
            if model_type == "ensemble":
                ...
                shap_function(img_tensor, models_cnn_loaded[0], class_names)
                ...
                shap_function(img_tensor, models_cnn_loaded[1], class_names)
                ...
                shap_function(img_tensor, models_cnn_loaded[2], class_names)
...
# Titre
    ...
    st.markdown(
                    ...
                    <p style='color:#000000; font-size:18px; font-weight:500; text-align: left'>
                        ...
                        D'autre part, notre projet démontre qu'il est tout à fait possible de renforcer la fiabilité et la précision d'une prédiction en combinant plusieurs modèles CNN : En effet, si un modèle CNN donné présentent des faiblesses à certains niveaux, elles peuvent être compensées par des modèles plus fiables sur ces points et inversement. Ce qui va améliorer globalement notre application.<br><br>
                    </p>                    
                    """,
                    unsafe_allow_html=True
                )
    ...
    cols = st.columns(3) # divise la ligne en 3 colonnes

    for n, col in enumerate(cols): # correspond au nb d'images par layer

        # Obtenir l'image avec la carte de chaleur superposée
        if n == 0 :
            mc_output = get_base64_of_image(rep_img_2 + "output_Resnet50.png")
            mc_model = "ResNet50"
            mc_accuracy = "93.25%"
        elif n == 1 :
            mc_output = get_base64_of_image(rep_img_2 + "output_EfficientnetB0.png")
            mc_model = "EfficientNetB0"
            mc_accuracy = "93.29%"
        else:
            mc_output = get_base64_of_image(rep_img_2 + "output_Nasnet.png")
            mc_model = "NasNet"
            mc_accuracy = "85.05%"
        ...
        with col:
            ...
            st.markdown(
                    f"""
                    <p style='color:#004B23; font-size:18px; font-weight:600;'>
    ...
    mc_output = get_base64_of_image(rep_img_2 + "output_Ensemble.png")
    mc_accuracy = "95.16%"

    st.markdown(
        f"""
        <p style='color:#004B23; font-size:22px; font-weight:600; ; text-align: center'>
            Ensemble
        </p>
        """,
        unsafe_allow_html=True
    ...
    st.markdown(
        f"""
        <p style='color:#004B23; font-size:18px; font-weight:600; ; text-align: center'>
            Accuracy : {mc_accuracy}
        </p>
        """,
        unsafe_allow_html=True
